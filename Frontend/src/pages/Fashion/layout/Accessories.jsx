...
const Accessories = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const data = await getProducts({ category: "accessories"});
      setProducts(data || []);
    } catch (error) {
  ...
  const handleAddToCart = async (product) => {
    ...
    return <div className="space-y-8">
      <div className="relative h-60 rounded-lg overflow-hidden">
        <img
        src="https://img.freepik.com/premium-photo/yellow-background-copy-space-with-black-smartphone-smartwatch-earphones-shop-advertisement-generative-ai_1219132-41597.jpg?semt=ais_user_personalization&w=740&q=80" 
        ...
        {filteredProducts.map((item) => (
          ...
          >
            ...
            />
             ...
             className="w-full bg-black text-white py-2 mt rounded"
             >Add To Cart</button>
          </div>
        ))}
      </div>
