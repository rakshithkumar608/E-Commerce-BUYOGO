...
const RunningSection = () => {
  const { filteredProducts, sort, setSort, minPrice, setMinPrice, maxPrice, setMaxPrice, handleAddToCart } = useProducts("running");

  return (
    <div className="bh-bg bh-animate-entrance min-h-screen">
      {/* ── HERO ── */}
      <div className="relative h-64 overflow-hidden bh-section-divider">
        <img src="https://images.unsplash.com/photo-1476480862126-209bfaa8edc8?w=1600" alt="Running" className="absolute inset-0 w-full h-full object-cover grayscale" />
        <div className="absolute inset-0" style={{ background: "#D02020", opacity: 0.85 }} />
        <div className="absolute inset-0 bh-dot-grid" />
        {/* Corner shapes */}
        <div className="absolute top-6 left-6 w-6 h-6 rounded-full bg-white opacity-60" />
        <div className="absolute bottom-6 right-6 w-6 h-6 bg-white opacity-60 rotate-45" />
        ...
        <div className="absolute inset-0 flex items-center px-8 md:px-16">
          <div className="space-y-3 text-white">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.25em]">Running</span>
            </div>
            <h1 className="bh-section-title text-4xl md:text-6xl text-white">
              Hit the Ground<br /><span style={{ color: "#F0C020" }}>Running</span>
            </h1>
      ...
      {/* ── FILTERS ── */}
      <div className="bh-section-divider px-6 md:px-12 py-4" style={{ background: "#F0C020" }}>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
        {filteredProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32" style={{ background: "white", border: "4px solid #121212", boxShadow: "8px 8px 0px 0px #121212" }}>
            <Zap size={48} style={{ color: "#D02020", opacity: 0.3 }} />
