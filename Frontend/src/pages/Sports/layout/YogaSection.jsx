...
const YogaSection = () => {
  const { filteredProducts, sort, setSort, minPrice, setMinPrice, maxPrice, setMaxPrice, handleAddToCart } = useProducts("yoga");

  return (
    <div className="bh-bg bh-animate-entrance min-h-screen">
      {/* ── HERO ── */}
      <div className="relative h-64 overflow-hidden bh-section-divider">
        <img src="https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=1600" alt="Yoga" className="absolute inset-0 w-full h-full object-cover grayscale" />
        <div className="absolute inset-0" style={{ background: "#D02020", opacity: 0.85 }} />
        <div className="absolute inset-0 bh-dot-grid" />
        <div className="absolute top-6 left-6 w-6 h-6 rotate-45 bg-white opacity-60" />
        <div className="absolute bottom-6 right-6 w-6 h-6 rounded-full bg-white opacity-60" />

        <div className="absolute inset-0 flex items-center px-8 md:px-16">
          <div className="space-y-3 text-white">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full flex items-center justify-center border-2 border-black bg-white" style={{ color: "#D02020" }}>
                <Heart size={22} />
              </div>
              <span className="text-[10px] font-bold uppercase tracking-[0.25em]">Yoga & Wellness</span>
            </div>
            <h1 className="bh-section-title text-4xl md:text-6xl text-white">
              Find Your<br /><span style={{ color: "#F0C020" }}>Balance</span>
            </h1>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
        {filteredProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32" style={{ background: "white", border: "4px solid #121212", boxShadow: "8px 8px 0px 0px #121212" }}>
            <Heart size={48} style={{ color: "#D02020", opacity: 0.3 }} />
            <p className="font-bold text-sm uppercase tracking-widest mt-4" style={{ color: "#888" }}>No products found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map((item, i) => (
              <SportsProductCard key={item._id} item={item} handleAddToCart={handleAddToCart} index={i} accentColor="#D02020" />
            ))}
          </div>
        )}
      </div>
